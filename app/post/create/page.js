...
function CreatePostForm() {
  ...
  // 1. Draft Persistence (like Facebook/Twitter)
  const [text, setText] = useState("");
  const [image, setImage] = useState("");          // Blob URL after upload
  const [imagePreview, setImagePreview] = useState(""); // local data URL for preview
  ...
  // 3. Hashtag Autocomplete Social Pattern
  ...
  const insertTag = (tagName) => {
    ...
    // Replace the current partial hashtag (e.g., #an -> #anime )
  ...
  // Detect Link for OG Preview
  useEffect(() => {
    // Only fetch if we DON'T have a manually uploaded image and NO poll
  ...
  // 2. BeforeUnload Guard (like Twitter)
  useEffect(() => {
  ...
  const handleImageSelect = useCallback(async (e) => {
    ...
    try {
      ...
      setImage(""); // Clear any previous final URL to signal a new upload is needed
    } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    // Auto-extract tags matching #[tagName] (Facebook style)
    const extractedTags = [];
    ...
    try {
      ...
      // If we have a local preview (data URL) but no finalImageUrl, upload it now.
      // This ensures we only upload the final chosen image to storage.
      if (imagePreview && imagePreview.startsWith("data:")) {
        ...
        finalImageUrl = url;
        setImage(url); // Update state just in case
      } else if (!imagePreview) {
        finalImageUrl = null;
      }

      // Build contentRef only if we have at least type and externalId from query params
      ...
      sessionStorage.removeItem("post-draft"); // Clear draft on success
  ...
  return (
    <main className="max-w-xl mx-auto p-4 sm:p-6 py-6 sm:py-10">
      ...
      <div className="bg-card border border-border sm:rounded-2xl rounded-xl shadow-sm overflow-hidden">
        {/* Post Reference Header (Posting About Anime/Character) */}
        {cr_title && (
        ...
        <form onSubmit={handleSubmit}>
          <div className="p-4 sm:p-5 flex gap-3 sm:gap-4">
            ...
            <div className="flex-1 min-w-0 flex flex-col pt-1">
              ...
              {/* Tag Suggestions Tray (Social Style) */}
              {tagTrayVisible && (
          ...
          {/* Action Toolbar (Twitter / Facebook style bottom rail) */}
          <div className="px-4 py-3 sm:px-5 border-t border-border flex flex-wrap gap-y-3 items-center justify-between bg-muted/10">
            ...
            <div className="flex items-center gap-3">
              {/* Character Counter (Standard in Twitter/Threads) */}
