...
export default function CoinTracker() {
  ...
  const updateCoins = (amount, type, event) => {
    ...
    if (isNaN(numCoins) || numCoins < 1 || (type === 'use' && numCoins > coins)) {
      alert(`Please enter a valid number of coins to ${type === 'add' ? 'add' : 'use'}.`);
      return false; // Indicate failure
    }

    const newCoinCount = type === 'add' ? coins + numCoins : coins - numCoins;
    setCoins(newCoinCount);

    setCharacterMood(type === 'add' ? "happy" : "sad");
    document.getElementById("character").classList.add(type === 'add' ? "animate__bounceIn" : "animate__shakeX");

    const coinElements = createCoinElements(numCoins);
    const clickX = event.clientX;
    const clickY = event.clientY;
    const from = type === 'add' ? { x: window.innerWidth - 50, y: 50 } : { x: clickX, y: clickY };
    const to = type === 'add' ? { x: clickX, y: clickY } : { x: window.innerWidth - 50, y: 50 };
    animateCoins(coinElements, from, to);
  ...
  const actions = [
    { label: "Use Advanced Options", type: "use", amount: 5, callback: () => console.log("Advanced options used!") },
    { label: "Share", type: "add", amount: 5, callback: () => console.log("Shared!") },
    { label: "Invite", type: "add", amount: 10, callback: () => console.log("Invited!") },
    { label: "Extra Coins", type: "add", amount: 2, callback: () => console.log("Extra coins!") },
    { label: "Penalty", type: "use", amount: 3, callback: () => console.log("Penalty applied!") },
    { label: "Big Win", type: "add", amount: 20, callback: () => console.log("Big Win!") },
    { label: "Small Loss", type: "use", amount: 1, callback: () => console.log("Small Loss!") },
  ];
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center p-6 overflow-hidden relative bg-gradient-to-r from-indigo-500 via-purple-500 to-pink-500">
      <div ref={coinContainerRef} className="absolute top-0 left-0 w-full h-full pointer-events-none z-50"></div>
      ...
      <div className="flex space-x-4 mb-4"> {/* Input fields for add/use */}
        <div>
          <input
            type="number"
            value={addCoinInput}
          ...
          />
          <button
            onClick={(e) => handleAction({ type: 'add', amount: parseInt(addCoinInput), callback: () => { } }, e)} // Directly use handleAction
        ...
        <div>
          ...
          />
          <button
            onClick={(e) => handleAction({ type: 'use', amount: parseInt(useCoinInput), callback: () => { } }, e)} // Directly use handleAction
      ...
      <div className="flex flex-col space-y-4"> {/* Action buttons */}
        {actions.map((action, index) => (
          <div key={index} className="flex items-center">
            <button
              onClick={(e) => handleAction(action, e)}
              className={`px-4 py-2 rounded-full shadow-md  ${action.type === 'add' ? 'bg-green-500 hover:bg-green-600 text-white' : 'bg-red-500 hover:bg-red-600 text-white'} transform hover:scale-105 transition duration-300`}
            >
