...
export async function buildContentSitemap(type) {
  ...
  // Fetch slug and updatedAt to provide accurate lastmod info
  const pages = await TopicPage.find({ type }, { slug: 1, updatedAt: 1 }).lean();

  const urls = pages
    .filter((page) => page.slug && !isAdult(page.slug))
    .map((page) => {
      // Set lower priority and frequency for characters vs main media
      const priority = type === "character" ? "0.5" : "0.7";
      const changefreq = type === "character" ? "monthly" : "weekly";
      const lastmod = (page.updatedAt || new Date()).toISOString();

      return `  <url>
    <loc>${BASE_URL}/${type}/${page.slug}</loc>
...
${urls.join("\n")}
  ...
  return new Response(xml, {
    headers: { 
      ...
      // Cache sitemap for 24 hours on the CDN to save function execution time
