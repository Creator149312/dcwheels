...
const RULES = [
  ...
  { pattern: /^\/api\/wheel\/suggest/, limit: 180, window: 60_000 },
  { pattern: /^\/api\/track-view/, limit: 120, window: 60_000 },

  // AI endpoints hit OpenAI on every call — each request costs real money.
  // Aggressive cap, shared across all /api/ai/* sub-paths. Combined with
  // the auth gate inside each route this prevents a logged-in user from
  // burning the budget in a tight loop.
  { pattern: /^\/api\/ai\//, limit: 15, window: 60_000 },

  // Admin/maintenance endpoints. Each route already gates on isAdminSession
  // but a per-IP cap is a cheap backstop against accidental loops.
  { pattern: /^\/api\/admin\//, limit: 60, window: 60_000 },
];
