...
// Steps: "choose" | "poll"
// Wheel goes straight to /dashboard — no step needed.

export default function TopicCreateFAB({ tag, tagDisplay, contentRef = null }) {
  ...
  return (
    <>
      ...
      <button
        onClick={openFAB}
        aria-label={`Create content in ${tagDisplay} Space`}
        className="fixed bottom-24 right-4 z-[75] w-14 h-14 rounded-full bg-primary text-primary-foreground shadow-lg shadow-primary/40 flex items-center justify-center hover:bg-primary/90 active:scale-95 transition-all md:bottom-8 md:right-8"
      >
            ...
            {step === "poll" && (
              <form onSubmit={handlePollSubmit} className="p-5 flex flex-col gap-4">
                ...
                <div className="flex flex-col gap-2">
                  ...
                  {options.length < 4 && (
                    <button
                      type="button"
                      onClick={addOption}
                      className="text-xs text-primary hover:underline self-start mt-1 font-medium"
                    >
                      + Add option
                    </button>
                  )}
                ...
                <button
                  type="submit"
                  disabled={loading}
                  className="w-full py-3 rounded-xl bg-primary text-primary-foreground font-semibold text-sm hover:bg-primary/90 disabled:opacity-60 transition-colors flex items-center justify-center gap-2"
                >
                  {loading ? (
                    <>
                ...
                {!session?.user && (
                  <p className="text-xs text-center text-muted-foreground">
                    You&apos;ll be asked to log in before posting.
                  </p>
                )}
              </form>
            )}
